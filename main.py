...
class WebsocketServices():
    def __init__(self):
        if os.getenv('WEBSOCKET') == "True":
            ...
            self.sender_thread = threading.Thread(target=empty_method,args=('WEBSOCKET',))
    def start(self):
...
class GamepadServices():
    def __init__(self):
        if os.getenv('GAMEPAD') == "True":
            ...
            self.sender_thread = threading.Thread(target=empty_method,args=('GAMEPAD',))
    def start(self):
        self.sender_thread.start()
        return self
    def end(self):
...
class HttpServer():
    def __init__(self):
        if os.getenv('HTTP') == "True":
            self.sender_thread = threading.Thread(target=http_server,args=(camera_quee,))
        else:
            self.sender_thread = threading.Thread(target=empty_method,args=('HTTP',))
    def start(self):
        self.sender_thread.start()
        return self
    def end(self):
...
class Camera():
    def __init__(self):
        if os.getenv('OPENCV') == "True":
            ...
            self.sender_thread = threading.Thread(target=empty_method,args=('OPENCV',))
    def start(self):
        self.sender_thread.start()
        return self
    def end(self):
