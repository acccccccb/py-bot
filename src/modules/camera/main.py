...
def video_stream(queue):
    ...
    global lock
    unknown_face_sub_dir = os.path.join(os.getcwd(), 'src', 'modules','camera','unknown_faces')
    known_face_sub_dir = os.path.join(os.getcwd(), 'src', 'modules','camera','known_faces', '*')
    known_face_encodings = [] #已知人脸 编码
    known_face_labels = [] # 已知人脸标签 就是文件夹名称
    tolerance = float(os.getenv('TOLERANCE')) # 匹配宽容度 0 - 1
    auto_save_unknown_face = False # 是否自动保存未知人脸
    auto_save_unknown_face_interval = 30 # 未知人脸保存间隔 秒
    ...
    def loadFace(known_faces_dir):
        ...
        image_files = glob.glob(known_faces_dir + "/*.jpg") + glob.glob(known_faces_dir + "/*.png")
    ...
    def drawText(text, x, y, draw):
        ...
        draw.text((x - 1, y), text, font=font, fill=stroke_color, stroke_width=stroke_width)
        draw.text((x + 1, y), text, font=font, fill=stroke_color, stroke_width=stroke_width)
        draw.text((x, y - 1), text, font=font, fill=stroke_color, stroke_width=stroke_width)
        draw.text((x, y + 1), text, font=font, fill=stroke_color, stroke_width=stroke_width)
    ...
    def match_faces(index,known_face_encoding, face_encoding, tolerance, face_queue):
        global lock
        matches = face_recognition.compare_faces(known_face_encoding, face_encoding, tolerance)
    ...
    async def main():
        ...
        while True:
            ...
            if len(face_encodings) > 0 :
                # 对每个检测到的人脸进行匹配
                for face_encoding_index, face_encoding in enumerate(face_encodings):
                    ...
                    for i, known_face_encoding in enumerate(known_face_encodings):
                        thread = threading.Thread(target=match_faces, args=(i, known_face_encoding, face_encoding, tolerance,face_queue))
